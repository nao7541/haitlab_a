        ...
        <BaseCard>
            <form @submit.prevent="postIdea">
                ...
                <div class="btns">
                    ...
                    <BaseButton type="button" class="btn save" @buttonEvent="saveIdea">Save</BaseButton>
                </div>
            </form>
        </BaseCard>
    </div>
...
export default {
    ...
    methods: {
        ...
        saveIdea() {
            ...
            // もしtitle, overviewが空欄なら空文字を入れる(nullだとpostできないため)
            if (!this.formData.title.isValid) {
                this.formData.title.val = ' ';
            }
            if (!this.formData.overview.isValid) {
                this.formData.overview.val = ' ';
            }

            this.registerIdea("save");
        },
        imageSelect(event) {
